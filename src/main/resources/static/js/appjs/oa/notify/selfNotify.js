				...
				columns : [
					...
					},
					{
						...
						formatter : function(value, row, index) {
							...
							if(row.proId > 0 && row.reviewId > 0) {
								e = '<a class="btn btn-primary btn-sm" href="javascript:page(\''+prefix+'/readProReview/'+row.id+'?reviewId='+row.reviewId+'&proId='+ row.proId + '&readonly=1&proType='+row.proType+'\',\'' + row.title + '\',\'' + 202102 + '\')" mce_href="#" title="打开"><i class="fa fa-book"></i></a> ';
							}
...
function read(id, deptId, proId, reviewId) {
	layer.open({
		type : 2,
		title : '查看',
		maxmin : true,
