var prefix = "/scienceProgressScience"
$(function () {
    load();
});
                    ...
                    },
                    {
                        ...
                        formatter: function (value, row, index) {
                            ...
                            var downloadAppendix = '<a class="btn btn-success   ' + s_download_zip_h + '" href="#" title="下载"  mce_href="#" onclick="downloadData(\''
                                ...
                                + '\',\'appendix'
                                + '\')">下载附件</a> ';
                            var downloadSupport = '<a class="btn btn-success   ' + s_download_zip_h + '" href="#" title="查看形式审查结果"  mce_href="#" onclick="viewCheckResult(\''
                                + row.proId
                                + '\',\''
                                + $("#proSubType").val()
                                + '\')">查看形式审查结果</a> ';
...
function printExcelProExt() {
    layer.open({
        ...
        area: ['800px', '520px'],
        content: prefix + "/printExcel" // iframe的url
    });
}
...
function view(proId, proType) {
    let url = '';
    let title = '';
    if (proType == 'science_progress') {
        url = '/scienceProgressScience/toEditChengguo?readonly=1&proId=' + proId;
        title = '科学技术奖成果';
    } else if (proType == 'science_personal') {
        url = '/chengguo_personal/apply_personal/edit?readonly=1&proId=' + proId;
        title = '先进个人成果';
    } else if (proType == 'science_team') {
        url = '/chengguo_team/apply_team/edit?readonly=1&proId=' + proId;
        title = '先进团队成果';
    }
...
function print(proId, protype, id) {
    ...
    if (protype == 'science_progress') {
        layer.confirm('确定要打印选中的记录？', {
            ...
            layer.open({
                ...
                shadeClose: false, // 点击遮罩关闭层
                area: ['800px', '520px'],
                content: '/chengguo_info/print?id=' + proId  // iframe的url
            })
...
function viewCheckResult(proId, proSubType) {
     ...
     if (proSubType == 'science_personal') {
         url = '/system/awardStyleValidatePerson/add?personId=0&';
         title = '个人形式审查';
     } else if (proSubType == 'science_team') {
         url = '/system/awardStyleValidateTeam/add?teamId=0&';
         title = '团队奖形式审查';
     } else if (proSubType == 'science_progress') {
         url = '/chengguo_info/toReview?';
         title = '科技奖形式审查';
     }
     if(url) {
        page(url + "proId=" + proId + "&proSubType=" + proSubType, title, 20220414, true);
...
function uploadFileData(proId, fileType) {
     ...
     parent.layer.open({
        zIndex:110,
	 	...
	 	shadeClose : false, // 点击遮罩关闭层
	 	area : [ '800px', '520px' ],
	 	content : '/award_flow/to_uploadsmall?proId='+proId+'&fileType='+fileType // iframe的url
	 });
}
