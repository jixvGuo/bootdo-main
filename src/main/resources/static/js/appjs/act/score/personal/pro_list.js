			...
			.bootstrapTable(
					{
						...
						url : prefix + "/personal/list", // 服务器数据的加载地址
					//	showRefresh : true,
					//	showToggle : true,
					//	showColumns : true,
						iconSize : 'outline',
						...
						columns : [
									...
									formatter : function(value, row, index) {
										...
										// var d = '<a class="btn btn-warning btn-sm" href="#" title="查看项目"  mce_href="#" onclick="onwatch(\''
										// 		+ row.id
										// 		+ '\')">查看项目</a> ';
...
function openstard() {
    layer.open({
        ...
        area : [ '800px', '520px' ],
        content : prefix + '/standard/personal' // iframe的url
    });
}
...
function onwatch(id, taskId) {
    layer.open({
        ...
        shadeClose : false, // 点击遮罩关闭层
        area : [ '800px', '520px' ],
        // content :  '/award_flow/publish_award_task/' + id+"/edit" // iframe的url
		content: '/chengguo_personal/apply_personal/edit?proId=' + id + "&taskId=" + taskId // iframe的url
    });
}
 ...
 * @param id
 */
function onscore(personId, proId) {
	console.log("评分personId=" + personId);
    layer.open({
        type : 2,
        title : '个人--打分',
        maxmin : true,
        shadeClose : false, // 点击遮罩关闭层
        area : [ '800px', '520px' ],
        content : prefix + '/toScoreProPersonal?personId='+ personId + "&proId=" + proId // iframe的url
    });
