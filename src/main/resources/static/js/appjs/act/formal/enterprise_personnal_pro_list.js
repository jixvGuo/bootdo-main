		...
		.bootstrapTable(
			{
				...
				queryParams : function(params) {
					return {
						...
						isValidate:true,//是否形式审查,true是，false不是
					// name:$('#searchName').val(),
				...
				columns : [
...

					{
						...
						formatter : function(value, row, index) {
							var e = '<a class="btn btn-primary" href="#" mce_href="#" title="查看信息" onclick="ondetail(\''
								+ row.id
								+ '\')"> 查看信息</a> ';

							var d = '<a class="btn btn-primary btn-sm" href="#" title="形式审查表"  mce_href="#" onclick="toFormalReview(\''
								+ row.id
								+ '\',\''+row.personalName+'\','
								+ row.personId
								+')">形式审查表</a> ';
...
function ondetail(id,taskId) {
    layer.open({
        ...
        shadeClose : false, // 点击遮罩关闭层
        area : [ '800px', '520px' ],
        content :  '/chengguo_personal/personalView?proId=' + id // iframe的url
    });
}
 ...
 * 个人形式审查表
 */
function toFormalReview(proId,proName,personId) {
    var toEditDocPage = layer.open({
        zIndex:100,
        type : 2,
        title : '个人形式审查-'+proName,
        maxmin : true,
        shadeClose : false, // 点击遮罩关闭层
        area : [ '800px', '520px' ],
        content : "/system/awardStyleValidatePerson/add?proId="+proId+"&personId="+personId // iframe的url
    });
