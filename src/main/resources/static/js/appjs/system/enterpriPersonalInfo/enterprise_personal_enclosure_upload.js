...
$().ready(function () {
    ...
    $(".btn-block").on("click",function () {
        ...
        var pid = $("#inputUserId", parent.document).val();

        if (pid.length == 0){
            parent.layer.msg("请先添加申报基本信息", {icon: 2,time:1800});

            return
        }
        console.log("用户ID" + pid);
        ...
        parent.layer.open({
	    	...
	    	area : [ '800px', '520px' ],
	    	content : prefix + '/to_uploadpersonal_small?proId='+pid+'&fileType='+fileType // iframe的url
	    });
    });
