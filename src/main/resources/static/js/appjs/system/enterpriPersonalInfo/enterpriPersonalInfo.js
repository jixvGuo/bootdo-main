                ...
                columns: [
...

                    {
                        ...
                        formatter: function (value, row, index) {
                            ...
                            var v =  '<a class="btn btn-primary" href="#" mce_href="#" title="查看" onclick="view(\''
                                + row.applyId
                                + '\','
                                + row.proId
                                + ')"> 查看</a> ';
                            ...
                            var e = '<a class="btn btn-primary ' + rs_edit_h + '" href="#" mce_href="#" title="修改" onclick="edit(\''
                                ...
                                + row.proId
                                + ')"> 修改</a> ';
                            var g = '<a class="btn btn-success '+ rs_print_h +'" href="#" title="打印"  mce_href="#" onclick="printApply(\''
                                + row.id
                                + '\')">打印 </a> ';

                            var review = '<a class="btn btn-primary ' + rs_review_h + '" href="javascript:page(\'/enterpriseUnitApply/toReview?situationId=' + row.situationId + '&proId=' + row.proId + '\',\'' + row.userName + '\',\'' + 202101 + '\')" mce_href="#" title="审核">审核</a> ';
                            ...
                            var reviewResult = '<a class="btn btn-primary ' + rs_review_result_h + '" href="javascript:page(\'/system/awardStyleValidatePerson/viewValidateResult?' + 'proId=' + row.proId + '&readonly=1\',\'' + row.userName + '\',\'' + 202102 + '\')" mce_href="#" title="审查结果">审查结果</a> ';
...
function cancelCheck(proId){
    layer.confirm('确定要撤回,重新修改吗？', {
        ...
        $.ajax({
            url: "/scienceProgressScience/personalCancelCheck",
            type: "post",
            data: {
                'proId': proId
            },
...
function view(id, proId) {
    console.log("id:" + id);
    console.log("proId:" + proId);
    let taskId = $("#publishTaskId").val();
	page('/chengguo_personal/apply_personal/edit?readonly=1&taskId=' + taskId + '&proId=' + proId, '个人查看', 20220515);
}
...
function edit(id, proId) {
    ...
    console.log("proId:" + proId);
    let taskId = $("#publishTaskId").val();
	page('/chengguo_personal/apply_personal/edit?readonly=0&taskId=' + taskId + '&proId=' + proId, '个人编辑', 20220515);
}
...
function printApply(id) {
    var applyPage = layer.open({
        ...
        shadeClose: false, // 点击遮罩关闭层
        area: ['800px', '520px'],
        content: "/system/enterpriPersonalInfo/print?id="+ id // iframe的url
    });
...
function printExcel() {
    var applyPage = layer.open({
        ...
        shadeClose: false, // 点击遮罩关闭层
        area: ['800px', '520px'],
        content: "/system/enterpriPersonalInfo/printExcel" // iframe的url
    });
    layer.full(applyPage)
}
/**
