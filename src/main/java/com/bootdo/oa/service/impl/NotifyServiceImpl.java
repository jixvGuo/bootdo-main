...
public class NotifyServiceImpl implements NotifyService {
     ...
     */
    @Override
    public int saveDeptNotify(AssDeptNotifyRecord assDeptNotifyRecord) {
        return notifyDao.saveDeptNotify(assDeptNotifyRecord);
    }
     ...
     */
    @Override
    public int saveProReviewNotify(AssNotifyProReviewRecord assNotifyProReviewRecord) {
        return notifyDao.saveProReviewNotify(assNotifyProReviewRecord);
    }

    @Override
    public int saveProReviewNotifyShip(long notifyId, int proId, int reviewId, String proType) {
        AssNotifyProReviewRecord assNotifyProReviewRecord = new AssNotifyProReviewRecord();
        assNotifyProReviewRecord.setNotifyId(notifyId);
        assNotifyProReviewRecord.setReviewId(reviewId);
        assNotifyProReviewRecord.setProId(proId);
        assNotifyProReviewRecord.setProType(proType);
        return saveProReviewNotify(assNotifyProReviewRecord);
    }
    ...
    public PageUtils selfList(Map<String, Object> map) {
        ...
        for (NotifyDTO notifyDTO : rows) {
            ...
            Long notifyId = notifyDTO.getId();
            int deptId = notifyDTO.getDeptId();
            String url = notifyDTO.getNotifyUrl() + (deptId > 0 ? deptId : notifyId);
            if(deptId > 0) {
                url = url.replaceAll("NOTIFY_ID", notifyId.toString());
            }
            notifyDTO.setNotifyUrl(url);
        }
