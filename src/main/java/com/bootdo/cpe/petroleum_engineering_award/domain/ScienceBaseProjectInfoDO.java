...
//    }
    ...
    public void initApplyStat() {
        ...
        boolean isReject = OilProStatEnum.REJECT.getProStat().equals(this.proStat);
        boolean isChecking = OilProStatEnum.CHECK.getProStat().equals(this.proStat);
        boolean isNotSubmitted = StringUtils.isBlank(this.proStat);
        boolean isReviewResultStat =
                OilProStatEnum.PARTAKE_AWARD.getProStat().equals(this.proStat)
                        || OilProStatEnum.REJECT.getProStat().equals(this.proStat)
                        || OilProStatEnum.NO_AWARD.getProStat().equals(this.proStat)
                        || OilProStatEnum.DELAYED_AWARD.getProStat().equals(this.proStat);
        ...
        // E. 申报结束、形审未开始（新增逻辑：企业显示项目申报列表按钮）
        if (applyEnded && !checkStarted) {
            // 申报时间结束但形审还未开始的状态
            this.isReviewResult = false;
